<template>
  <div class="bee-main">
    <div class="assert ">
      <div class="am-collapse am-topbar-collapse titleBar"  data-am-sticky="{animation: 'slide-top'}"
           data-am-scrollspy="{animation: 'fade'}">
        <h1 class="am-topbar-brand">永远的蝴蝶</h1>
        <div class="am-topbar-right">
          <button data-v-2f178667="" class="am-btn am-btn-primary am-topbar-btn am-btn-sm">赞TA</button>
          <button data-v-2f178667="" class="am-btn am-btn-secondary am-topbar-btn am-btn-sm">收集标签</button>
        </div>
      </div>
    ...
    <!--<form>-->
      <!--<div class="detail">-->
        <!--<div class="types_choose">-->
          <!--<a href="#" :id="item._id" :data-name="item.name" v-for="item in tag" @click.prevent="tagIt($event)"-->
             <!--:class="tags.indexOf(item._id)>=0?'active':''" v-show="(!isEdit&&tags.indexOf(item._id)>=0)||isEdit">{{item.name}}</a>-->
          ...
          this.$http.post('/api/like', {
            ...
            .then(res => {
              ...
              if (res.data.code == '11005') {
                this.$refs.likebox.classList.add('like');
